...
namespace ValveResourceFormat
{
	public class Package : IDisposable
	{
		...
		public void ReadEntry(PackageEntry entry, out byte[] output, bool validateCrc = true)
		{
			checked
			{
				output = new byte[unchecked((long)entry.SmallData.Length) + unchecked((long)entry.Length)];
				...
				if (entry.Length != 0)
				{
					...
					try
					{
						...
						if (entry.ArchiveIndex != 32767)
						{
							...
							string path = $"{FileName}_{entry.ArchiveIndex:D3}.vpk";
			...
			while (true)
			{
				...
				while (true)
				{
					...
					while (true)
					{
						...
						packageEntry.FileName = text3;
						packageEntry.DirectoryName = text2;
						packageEntry.TypeName = text;
						packageEntry.CRC32 = Reader.ReadUInt32();
						packageEntry.SmallData = new byte[Reader.ReadUInt16()];
						packageEntry.ArchiveIndex = Reader.ReadUInt16();
						packageEntry.Offset = Reader.ReadUInt32();
						packageEntry.Length = Reader.ReadUInt32();
						PackageEntry packageEntry2 = packageEntry;
						if (Reader.ReadUInt16() != 65535)
						{
							throw new FormatException("Invalid terminator.");
						}
						if (packageEntry2.SmallData.Length != 0)
						{
							Reader.Read(packageEntry2.SmallData, 0, packageEntry2.SmallData.Length);
						}
						list.Add(packageEntry2);
					}
				}
		...
		public void VerifyHashes()
		{
			...
			checked
			{
				using (MD5 mD = MD5.Create())
				{
					...
					byte[] array = mD.ComputeHash(Reader.ReadBytes((int)(HeaderSize + TreeSize + FileDataSectionSize + ArchiveMD5SectionSize + 32u)));
		...
		public bool IsSignatureValid()
		{
			...
			byte[] rgbHash = new SHA256Managed().ComputeHash(Reader.ReadBytes(checked((int)(HeaderSize + TreeSize + FileDataSectionSize + ArchiveMD5SectionSize + OtherMD5SectionSize))));
