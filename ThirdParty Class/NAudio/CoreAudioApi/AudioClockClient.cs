...
namespace NAudio.CoreAudioApi
{
	public class AudioClockClient : IDisposable
	{
		...
		public int Characteristics
		{
			get
			{
				Marshal.ThrowExceptionForHR(audioClockClientInterface.GetCharacteristics(out uint characteristics));
		...
		public ulong Frequency
		{
			get
			{
				Marshal.ThrowExceptionForHR(audioClockClientInterface.GetFrequency(out ulong frequency));
				return frequency;
			}
		}
		...
		public ulong AdjustedPosition
		{
			get
			{
				...
				if (Stopwatch.IsHighResolution)
				{
					...
					ulong num4 = (num3 - qpcPosition) / 100uL;
					ulong num5 = num4 / num;
